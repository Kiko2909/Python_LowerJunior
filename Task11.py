...
while days < 1 or days > 100 :
    days = int (input ("Какое количество дней мы рассматриваем? "))

print ("Допустимые значение среднесуточной температурв от -50 до 50 градусов.")

for i in range(1, days + 1) :
    temp = -100
    while temp < -50 or temp > 50 :
        temp = int(input("Введите температуру дня: "))
    if temp > 0:
        sum +=1
        if total < sum :
            total = sum
...
print ("Самая длинная оттепель длилась ", total," (в днях)")
